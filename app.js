...
sendBtn.addEventListener('click', async () => {
    ...
    if (message) {
        appendMessage(message, 'user-message', 'You', 'img/user.jpg');
        userInput.value = '';

        try {
            const chatGptResponse = await fetchChatGptResponse([{ role: 'user', content: message }]);
            appendMessage(chatGptResponse, 'gpt-message', 'ThinkFast AI - DucAnh', 'img/robot.png');
        } catch (error) {
            console.error('Error in sendBtn event listener:', error);
            alert('An error occurred while fetching the server response');
        }
    }
});
